...
for (const m of modules) {
  const file = m.startsWith("ui_integration") || m.startsWith("ui_downloads") || m.startsWith("ui_ngl") || m.startsWith("ui_footnotes") || m.startsWith("ui_sensitivity") ? `tests/${m}.tsx` : `tests/${m}.ts`;
