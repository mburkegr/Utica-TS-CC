 ...
 */
export function UnitSearch({ status, operators, onNeedLayer, onPick }: UnitSearchProps) {
  const [query, setQuery] = React.useState("");
  const [active, setActive] = React.useState(0);
  const data = status.state === "ready" ? status.data : null;
  const index = React.useMemo(() => (data ? buildUnitIndex(data) : []), [data]);
  const hits = React.useMemo(() => searchUnits(index, query, undefined, operators), [index, query, operators]);
  ...
  const note =
    ...
    : searching && status.state === "ready" && hits.length === 0 ? (operators.length === 1 ? `No matching unit for ${operators[0]}` : operators.length > 1 ? `No matching unit for the ${operators.length} selected operators` : "No matching unit")
    : "";

  return (
    <div className="gis-search" data-testid="unit-search">
      <input
        type="search" className="gis-search-input" value={query} placeholder={operators.length === 1 ? `Find a ${operators[0]} unit` : operators.length > 1 ? `Find a unit (${operators.length} operators)` : "Find a unit, operator or order no."}
